...
function ProductCard({p}){
 const images=imageList(p.main_image_url);
 return <Link className="card" to={`/product/${p.id}`}><div className="photo">{images[0]?<img src={images[0]} alt={p.name}/>:<span>PRODUCT IMAGE</span>}</div><small>{p.brand?.name||p.category?.name}</small><h3>{p.name}</h3><strong>{money(p.price)}</strong></Link>
}

function Product(){
 const {id}=useParams(),nav=useNavigate();const [p,setP]=useState(null);const [variants,setVariants]=useState([]);const [selected,setSelected]=useState(null);const [qty,setQty]=useState(1);const [activeImage,setActiveImage]=useState(0);
 useEffect(()=>{(async()=>{let {data}=await supabase.from("products").select("*,category:categories(name),brand:brands(name)").eq("id",id).maybeSingle();if(!data)data=demo.find(x=>x.id===id);setP(data);setActiveImage(0);if(data){let r=await supabase.from("product_variants").select("*").eq("product_id",id).eq("is_active",true);setVariants(r.data||[]);}})()},[id]);
 if(!p)return <section className="section"><p>Loading...</p></section>;
 const images=imageList(p.main_image_url);
 const add=()=>{const c=readCart(), key=`${p.id}-${selected?.id||"base"}`,found=c.find(x=>x.key===key),item={key,product_id:p.id,name:p.name,price:selected?.price??p.price,quantity:qty,variant:selected?.variant_name||"",image:images[0]||""};found?found.quantity+=qty:c.push(item);saveCart(c);nav("/cart")};
 return <section className="section product"><button className="back" onClick={()=>nav(-1)}><ArrowLeft/> Back</button><div className="productLayout"><div className="productGallery"><div className="largePhoto">{images[activeImage]?<img src={images[activeImage]} alt={p.name}/>:<span>PRODUCT IMAGE</span>}</div>{images.length>1&&<div className="galleryThumbs">{images.map((url,i)=><button type="button" key={url+i} onClick={()=>setActiveImage(i)} className={i===activeImage?"activeGalleryThumb":""}><img src={url} alt=""/></button>)}</div>}</div><div><p className="eyebrow">{p.brand?.name||p.category?.name}</p><h1>{p.name}</h1><h2>{money(selected?.price??p.price)}</h2><p>{p.description}</p>{variants.length>0&&<div><h3>Choose an option</h3><div className="variantList">{variants.map(v=><button className={selected?.id===v.id?"selected":""} onClick={()=>setSelected(v)} key={v.id}>{v.variant_name||[v.size,v.storage,v.colour,v.condition].filter(Boolean).join(" / ")}</button>)}</div></div>}<div className="quantity"><button onClick={()=>setQty(Math.max(1,qty-1))}><Minus/></button><b>{qty}</b><button onClick={()=>setQty(qty+1)}><Plus/></button></div><button className="btn full" onClick={add}>Add to Cart</button></div></div></section>
}
